...
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "../src/sdcard_test.c" 2
# 13 "../src/sdcard_test.c"
...
# 14 "../src/sdcard_test.c" 2

# 1 ".././src/timing.h" 1
# 10 ".././src/timing.h"
unsigned int get_time(void);
# 15 "../src/sdcard_test.c" 2
...
int sdcard_test(void)
{
    ...
    for( i = 0; i < sizeof(Buff); i++) Buff[i] = i + i / 512;
    ...
    rc = f_open(&Fil, "Data.bin", 0x02 | 0x08);
    if(rc) die(rc);
    printf("done.\n");
...
static char lfn[255 + 1];
    ...
    for(;;)
    {
        ...
        printf("\nreaddir sucess...");
        if(fno.fattrib & 0x10) printf("   <dir>  %s\n", fno.fname);
        else printf("%8d  %s\n", fno.fsize, fno.fname);
    }
    if(rc) die(rc);
